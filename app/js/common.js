...
$('#calc-form').change(function(){
	...
	let allWidth = document.getElementsByClassName('width'),
		allHeight = document.getElementsByClassName('height'),
		result = document.getElementsByClassName('result')[0],
		size = 0;

	for (i=0; i<allWidth.length; i++) {
		let w = $(allWidth[i]).val(),
			h = $(allHeight[i]).val(),
			p = 0;
